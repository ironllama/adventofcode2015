...
$points_leader = '';
$points_leader_has = 0;
foreach (range(1, 2503) as $sec) {
    // foreach (range(1, 1000) as $sec) {
    $winner = at_seconds($sec);
    ++$reindeer[$winner]['points'];

    if ($reindeer[$winner]['points'] > $points_leader_has) {
        $points_leader_has = $reindeer[$winner]['points'];
        $points_leader = $winner;
    }
}

echo "POINTS: ${points_leader_has} ${points_leader}".PHP_EOL;
