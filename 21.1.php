...
$cheapest_cost = 0;
$cheapest_combo = [];
// Brute force through all the combinations!
        ...
        foreach ($rings as $one_ring) {
            foreach ($rings as $two_ring) {
                ...
                if ($turns_to_win <= $turns_to_lose) {
                    ...
                    if (0 === $cheapest_cost || $cost < $cheapest_cost) {
                        $cheapest_cost = $cost;
                        $cheapest_combo = [$my_weapon, $my_armor, $one_ring, $two_ring];
                    }
...
echo "CHEAPEST: {$cheapest_cost}".PHP_EOL;
print_r($cheapest_combo);
